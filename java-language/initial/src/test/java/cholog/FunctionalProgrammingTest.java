...
public class FunctionalProgrammingTest {
    ...
    class SyntaxTest {
        ...
        void User의_나이를_기준으로_정렬한다() {
            ...
            final List<User> usersForStream = new ArrayList<>(List.of(brown, neo, brie));
            final var streamValue = usersForStream.stream()
            ...
            assertAll(
                    () -> assertThat(streamValue).last().isSameAs(neo),
                    () -> assertThat(streamValue).first().isSameAs(brie)
            );
        }
    }
    ...
    class StreamTest {
        ...
        void 명령형으로_작성된_코드를_선언형으로_변경한다() {
            ...
            assertThat(crew).isEqualTo(new Crew("김재", "th", 29));
        }
        ...
        void Stream_API를_활용하여_전쟁과_평화_내용_중_문자_길이가_12보다_큰_경우의_수를_구한다() throws IOException {
            ...
            final var actual = Arrays.stream(words)
                    .filter(w -> w.length() > 12)
                    .count();

            assertThat(actual).isEqualTo(1_946);
        }
         ...
         * <p>
         * ## 조건 - 단어의 길이가 12자를 초과하는 단어를 추출한다. - 12자가 넘는 단어 중 길이가 짧은 순서대로 100개의 단어를 추출한다. -- 단어 중복을 허용하지 않는다. 즉, 서로 다른단어
         * 100개를 추출한다. - 추출한 단어의 두번째 문자가 대문자라면 단어를 제외한다. -- ex) abc 허용 | abC 허용 | aBc 제외 | aBC 제외 | AbC 허용 - 추출한 단어를
        ...
        void 전쟁과_평화_내용_중_아래_조건에_해당하는_단어를_찾아서_추출한다() throws IOException {
            ...
            final var words = contents.split("\\P{L}+");
            final var results = Arrays.stream(words)
                    .filter(val -> val.length() > 12)
                    .sorted((o1, o2) -> o1.length() - o2.length())
                    .distinct()
                    .limit(100)
                    .filter(val -> !Character.isUpperCase(val.charAt(1)))
